...
(function () {
    ...
    const P = {
        ...
        "Tables d'équivalences déclarées : homonymes et variantes de translittération (Mohammad / Mohammed / Muhammad), équivalents entre langues (Henri / Harry / Heinrich), exonymes de villes et de pays (Londres / London, Allemagne / Germany / DE). Aucune métrique de chaîne ne peut déduire ces rapprochements : ils se déclarent. Les équivalences agissent au blocking (les termes équivalents deviennent candidats) et au scoring. Elles élargissent mécaniquement le périmètre des alertes : prénoms et noms sont actifs à la livraison — impact mesuré, +6 vrais positifs et 0 faux positif sur un panel de 716 clients — tandis que villes, pays et états restent inactifs faute de mesure. Chiffrez toute activation avec Mesurer l'impact avant de produire.": {"en": "Declared equivalence tables: homonyms and transliteration variants (Mohammad / Mohammed / Muhammad), cross-language equivalents (Henri / Harry / Heinrich), city and country exonyms (Londres / London, Allemagne / Germany / DE). No string metric can infer these matches: they are declared. Equivalences act on blocking (equivalent terms become candidates) and on scoring. They mechanically widen the alert perimeter: given names and surnames are active on delivery — measured impact, +6 true positives and 0 false positives on a 716-client panel — while cities, countries and states stay inactive for want of a measurement. Quantify any activation with Measure impact before going to production.", "de": "Deklarierte Äquivalenztabellen: Homonyme und Transliterationsvarianten (Mohammad / Mohammed / Muhammad), sprachübergreifende Entsprechungen (Henri / Harry / Heinrich), Exonyme von Städten und Ländern (Londres / London, Allemagne / Germany / DE). Keine Zeichenkettenmetrik kann diese Zuordnungen ableiten: Sie werden deklariert. Äquivalenzen wirken beim Blocking (äquivalente Begriffe werden Kandidaten) und beim Scoring. Sie erweitern den Alarmumfang zwangsläufig: Vornamen und Nachnamen sind bei Auslieferung aktiv — gemessene Wirkung, +6 echte Treffer und 0 Fehltreffer bei 716 Kunden —, während Städte, Länder und Regionen mangels Messung inaktiv bleiben. Beziffern Sie jede Aktivierung mit „Wirkung messen“ vor der Produktion.", "es": "Tablas de equivalencias declaradas: homónimos y variantes de transliteración (Mohammad / Mohammed / Muhammad), equivalentes entre lenguas (Henri / Harry / Heinrich), exónimos de ciudades y países (Londres / London, Allemagne / Germany / DE). Ninguna métrica de cadena puede deducir estos acercamientos: se declaran. Las equivalencias actúan en el bloqueo (los términos equivalentes pasan a ser candidatos) y en la puntuación. Amplían mecánicamente el perímetro de alertas: nombres y apellidos están activos de fábrica — impacto medido, +6 verdaderos positivos y 0 falsos positivos sobre un panel de 716 clientes — mientras que ciudades, países y estados siguen inactivos a falta de medición. Cuantifique toda activación con Medir el impacto antes de producir.", "zh": "已声明的等价表：同音异形与音译变体（Mohammad / Mohammed / Muhammad）、跨语言对应（Henri / Harry / Heinrich）、城市与国家的外来名（Londres / London、Allemagne / Germany / DE）。任何字符串度量都无法推导出这些关联：它们只能声明。等价同时作用于阻断（等价词成为候选）与评分。它们必然扩大警报范围：名与姓在交付时即为启用——影响已量化，716 名客户样本中新增 6 个真阳性、0 个假阳性——城市、国家与省州因未经量化而保持停用。投产前请先用“量化影响”评估任何启用。", "ar": "جداول التكافؤ المُعلنة: المتشابهات وصيغ النقل الحرفي (Mohammad / Mohammed / Muhammad)، المكافئات بين اللغات (Henri / Harry / Heinrich)، الأسماء الأجنبية للمدن والبلدان (Londres / London، Allemagne / Germany / DE). لا يمكن لأي مقياس نصي استنتاج هذه المقاربات: إنها تُعلَن. تعمل التكافؤات على الحجب (تصبح المصطلحات المكافئة مرشحة) وعلى التنقيط. توسّع حتماً نطاق التنبيهات: الأسماء الشخصية وأسماء العائلة مفعّلة عند التسليم — أثر مقيس، ‏6 إصابات صحيحة إضافية و0 إنذار كاذب على عيّنة من 716 عميلاً — بينما تبقى المدن والبلدان والولايات معطّلة لعدم قياسها. قِس أثر أي تفعيل عبر «قياس الأثر» قبل الإنتاج."},
        "Chaque nuit, le moteur cherche de nouveaux homonymes dans deux sources que l'installation possède déjà. Les alias déclarés par les listes officielles : quand une source affirme qu'une fiche « Mohammad AL-ASSAD » porte l'alias « Mohammed AL-ASAD », l'autorité elle-même établit que les deux graphies désignent la même personne. Les alertes confirmées en revue : un analyste a validé humainement que deux écritures désignent le même individu. Garde-fou décisif : les deux noms doivent avoir le même nombre de mots et ne différer que sur un seul — « Ali Hassan » et « Abu Muhammad » divergent sur deux mots et sont écartés, car c'est un surnom et non une variante d'écriture. Chaque découverte porte ses preuves et reste révocable.": {"en": "Every night the engine looks for new homonyms in two sources the installation already owns. Aliases declared by the official lists: when a source states that a record “Mohammad AL-ASSAD” carries the alias “Mohammed AL-ASAD”, the authority itself establishes that both spellings designate the same person. Alerts confirmed in review: an analyst has humanly validated that two spellings designate the same individual. The decisive safeguard: both names must have the same number of words and differ on only one — “Ali Hassan” and “Abu Muhammad” differ on two words and are discarded, because that is a nickname, not a spelling variant. Every discovery carries its evidence and stays revocable.", "de": "Jede Nacht sucht die Engine neue Homonyme in zwei Quellen, die die Installation bereits besitzt. Von den offiziellen Listen deklarierte Aliasnamen: Wenn eine Quelle angibt, dass ein Eintrag „Mohammad AL-ASSAD“ den Alias „Mohammed AL-ASAD“ trägt, stellt die Behörde selbst fest, dass beide Schreibweisen dieselbe Person bezeichnen. In der Prüfung bestätigte Alarme: Ein Analyst hat menschlich validiert, dass zwei Schreibweisen dieselbe Person bezeichnen. Die entscheidende Absicherung: Beide Namen müssen gleich viele Wörter haben und sich in nur einem unterscheiden — „Ali Hassan“ und „Abu Muhammad“ unterscheiden sich in zwei Wörtern und werden verworfen, denn das ist ein Spitzname und keine Schreibvariante. Jede Entdeckung trägt ihre Belege und bleibt widerrufbar.", "es": "Cada noche el motor busca nuevos homónimos en dos fuentes que la instalación ya posee. Los alias declarados por las listas oficiales: cuando una fuente afirma que una ficha «Mohammad AL-ASSAD» lleva el alias «Mohammed AL-ASAD», la propia autoridad establece que ambas grafías designan a la misma persona. Las alertas confirmadas en revisión: un analista ha validado humanamente que dos grafías designan al mismo individuo. Salvaguarda decisiva: ambos nombres deben tener el mismo número de palabras y diferir en una sola — «Ali Hassan» y «Abu Muhammad» difieren en dos palabras y se descartan, porque es un apodo y no una variante de escritura. Cada hallazgo lleva sus pruebas y sigue siendo revocable.", "zh": "每晚，引擎都会在本地已有的两类来源中寻找新的同名异形词。官方名单自行声明的别名：当来源指明「Mohammad AL-ASSAD」一条记录带有别名「Mohammed AL-ASAD」时，是当局本身确认两种拼写指向同一个人。复核中确认的警报：分析员已人工确认两种写法指向同一个体。关键防线：两个姓名的词数必须相同，且只能有一个词不同——「Ali Hassan」与「Abu Muhammad」有两个词不同，予以剔除，因为那是别号而非拼写变体。每项发现都附带证据，且始终可撤销。", "ar": "كل ليلة يبحث المحرك عن متشابهات جديدة في مصدرين تملكهما المنشأة بالفعل. الأسماء المستعارة التي تعلنها القوائم الرسمية: حين تفيد المصدر بأن سجل «Mohammad AL-ASSAD» يحمل الاسم المستعار «Mohammed AL-ASAD»، فإن السلطة نفسها تقرّر أن الصيغتين تشيران إلى الشخص ذاته. التنبيهات المؤكدة في المراجعة: محلل تحقّق بشرياً من أن صيغتين تشيران إلى الفرد نفسه. الضمانة الحاسمة: يجب أن يحمل الاسمان العدد نفسه من الكلمات وألا يختلفا إلا في كلمة واحدة — «Ali Hassan» و«Abu Muhammad» يختلفان في كلمتين فيُستبعدان، لأن ذلك لقب لا صيغة كتابة. كل اكتشاف يحمل أدلته ويظل قابلاً للإلغاء."},
        ...
        "Trafic reçu sur les points d'entrée /api/hooks/screening et /api/hooks/client-upsert : volumes, appelants et erreurs (signature refusée, charge invalide). Toute livraison est tracée, même sans en-tête X-Idempotency-Key. Ni la clé d'idempotence ni la réponse ne sont affichées ici.": {"en": "Traffic received on the /api/hooks/screening and /api/hooks/client-upsert entry points: volumes, callers and errors (rejected signature, invalid payload). Every delivery is recorded, even without an X-Idempotency-Key header. Neither the idempotency key nor the response is shown here.", "de": "Auf den Endpunkten /api/hooks/screening und /api/hooks/client-upsert eingegangener Verkehr: Volumen, Aufrufer und Fehler (abgelehnte Signatur, ungültige Nutzlast). Jede Zustellung wird protokolliert, auch ohne X-Idempotency-Key-Header. Weder der Idempotenzschlüssel noch die Antwort werden hier angezeigt.", "es": "Tráfico recibido en los puntos de entrada /api/hooks/screening y /api/hooks/client-upsert: volúmenes, llamantes y errores (firma rechazada, carga inválida). Toda entrega queda registrada, incluso sin la cabecera X-Idempotency-Key. Aquí no se muestran ni la clave de idempotencia ni la respuesta.", "zh": "/api/hooks/screening 与 /api/hooks/client-upsert 入口收到的流量：调用量、调用方与错误（签名被拒、载荷无效）。即使没有 X-Idempotency-Key 请求头，每次投递也会被记录。此处不展示幂等键，也不展示响应内容。", "ar": "حركة المرور الواردة على نقطتي /api/hooks/screening و/api/hooks/client-upsert: الأحجام والمستدعون والأخطاء (توقيع مرفوض، حمولة غير صالحة). تُسجَّل كل عملية تسليم حتى بدون ترويسة X-Idempotency-Key. لا يُعرض هنا مفتاح عدم التكرار ولا الاستجابة."},
        ...
        "Un mail à chaque étape de la production des listes, du criblage et du filtrage. Les étapes structurantes partent immédiatement ; les étapes à fort volume sont regroupées dans un récapitulatif périodique. Les destinataires sont déduits des rôles (adresse email du compte), avec repli sur les variables SMTP globales.": {"en": "An email at every step of list production, screening and filtering. Structural steps are sent immediately; high-volume steps are bundled into a periodic summary. Recipients are derived from roles (the account's email address), falling back to the global SMTP variables.", "de": "Eine E-Mail bei jedem Schritt der Listenproduktion, des Screenings und des Filterns. Strukturelle Schritte gehen sofort raus; Schritte mit hohem Volumen werden in einer periodischen Zusammenfassung gebündelt. Empfänger ergeben sich aus den Rollen (E-Mail-Adresse des Kontos), ersatzweise aus den globalen SMTP-Variablen.", "es": "Un correo en cada etapa de la producción de listas, del cribado y del filtrado. Las etapas estructurantes se envían de inmediato; las de gran volumen se agrupan en un resumen periódico. Los destinatarios se deducen de los roles (dirección de correo de la cuenta), con repliegue en las variables SMTP globales.", "zh": "名单生产、筛查与过滤的每个环节都发送邮件。关键环节立即发出；高频环节汇总为周期性摘要。收件人依据角色（账户邮箱地址）确定，缺省时回退到全局 SMTP 变量。", "ar": "بريد إلكتروني عند كل مرحلة من إنتاج القوائم والفحص والترشيح. المراحل الهيكلية تُرسل فوراً؛ أما المراحل عالية الحجم فتُجمّع في ملخص دوري. يُستنتج المستلمون من الأدوار (بريد الحساب)، مع الرجوع إلى متغيرات SMTP العامة."},
        "Complétude des champs KYC du référentiel clients en production : un dossier incomplet (date de naissance ou pays manquants) dégrade la précision du criblage et augmente les faux positifs. Barres vertes ≥ 95 %, orange ≥ 80 %, rouges en dessous. Un score minimal peut être exigé dans les réglages de gouvernance : chaque import qui passe sous ce seuil déclenche une notification.": {"en": "Completeness of the KYC fields in the production client repository: an incomplete file (missing date of birth or country) degrades screening precision and increases false positives. Green bars ≥ 95%, orange ≥ 80%, red below. A minimum score can be required in the governance settings: every import that falls below that threshold triggers a notification.", "de": "Vollständigkeit der KYC-Felder im produktiven Kundenbestand: eine unvollständige Akte (fehlendes Geburtsdatum oder Land) verschlechtert die Screening-Präzision und erhöht die Fehlalarme. Grüne Balken ≥ 95 %, orange ≥ 80 %, rot darunter. In den Governance-Einstellungen kann ein Mindestscore verlangt werden: jeder Import, der diesen Schwellenwert unterschreitet, löst eine Benachrichtigung aus.", "es": "Completitud de los campos KYC del repositorio de clientes en producción: un expediente incompleto (fecha de nacimiento o país ausentes) degrada la precisión del cribado y aumenta los falsos positivos. Barras verdes ≥ 95 %, naranjas ≥ 80 %, rojas por debajo. Se puede exigir una puntuación mínima en los ajustes de gobernanza: cada importación que caiga por debajo de ese umbral genera una notificación.", "zh": "在产客户库 KYC 字段的完整度：档案不完整（缺出生日期或国家）会降低筛查精度并增加误报。绿色条 ≥ 95%，橙色 ≥ 80%，其下为红色。 可在治理设置中要求最低分数：任何低于该阈值的导入都会触发通知。", "ar": "اكتمال حقول اعرف عميلك في مستودع العملاء المنشور: الملف الناقص (تاريخ ميلاد أو بلد مفقود) يقلل دقة الفحص ويزيد الإنذارات الكاذبة. الأشرطة الخضراء ≥ 95%، البرتقالية ≥ 80%، والحمراء دون ذلك. يمكن اشتراط حد أدنى للنتيجة في إعدادات الحوكمة: كل استيراد يهبط دون هذا الحد يطلق إشعارًا."},
