...
(function () {
    ...
    const P = {
        ...
        "Tables d'équivalences déclarées : homonymes et variantes de translittération (Mohammad / Mohammed / Muhammad), équivalents entre langues (Henri / Harry / Heinrich), exonymes de villes et de pays (Londres / London, Allemagne / Germany / DE). Aucune métrique de chaîne ne peut déduire ces rapprochements : ils se déclarent. Les équivalences agissent au blocking (les termes équivalents deviennent candidats) et au scoring. Elles élargissent mécaniquement le périmètre des alertes : elles sont désactivées par défaut et doivent être mesurées au cahier de tests avant mise en production.": {"en": "Declared equivalence tables: homonyms and transliteration variants (Mohammad / Mohammed / Muhammad), cross-language equivalents (Henri / Harry / Heinrich), city and country exonyms (Londres / London, Allemagne / Germany / DE). No string metric can infer these matches: they are declared. Equivalences act on blocking (equivalent terms become candidates) and on scoring. They mechanically widen the alert perimeter: they are disabled by default and must be measured in the test book before going to production.", "de": "Deklarierte Äquivalenztabellen: Homonyme und Transliterationsvarianten (Mohammad / Mohammed / Muhammad), sprachübergreifende Entsprechungen (Henri / Harry / Heinrich), Exonyme von Städten und Ländern (Londres / London, Allemagne / Germany / DE). Keine Zeichenkettenmetrik kann diese Zuordnungen ableiten: Sie werden deklariert. Äquivalenzen wirken beim Blocking (äquivalente Begriffe werden Kandidaten) und beim Scoring. Sie erweitern den Alarmumfang zwangsläufig: standardmäßig deaktiviert und vor der Produktion im Testbuch zu messen.", "es": "Tablas de equivalencias declaradas: homónimos y variantes de transliteración (Mohammad / Mohammed / Muhammad), equivalentes entre lenguas (Henri / Harry / Heinrich), exónimos de ciudades y países (Londres / London, Allemagne / Germany / DE). Ninguna métrica de cadena puede deducir estos acercamientos: se declaran. Las equivalencias actúan en el bloqueo (los términos equivalentes pasan a ser candidatos) y en la puntuación. Amplían mecánicamente el perímetro de alertas: están desactivadas por defecto y deben medirse en el cuaderno de pruebas antes de pasar a producción.", "zh": "已声明的等价表：同音异形与音译变体（Mohammad / Mohammed / Muhammad）、跨语言对应（Henri / Harry / Heinrich）、城市与国家的外来名（Londres / London、Allemagne / Germany / DE）。任何字符串度量都无法推导出这些关联：它们只能声明。等价同时作用于阻断（等价词成为候选）与评分。它们必然扩大警报范围：默认停用，投产前须在测试手册中量化其偏差。", "ar": "جداول التكافؤ المُعلنة: المتشابهات وصيغ النقل الحرفي (Mohammad / Mohammed / Muhammad)، المكافئات بين اللغات (Henri / Harry / Heinrich)، الأسماء الأجنبية للمدن والبلدان (Londres / London، Allemagne / Germany / DE). لا يمكن لأي مقياس نصي استنتاج هذه المقاربات: إنها تُعلَن. تعمل التكافؤات على الحجب (تصبح المصطلحات المكافئة مرشحة) وعلى التنقيط. توسّع حتماً نطاق التنبيهات: معطّلة افتراضياً ويجب قياسها في كراسة الاختبارات قبل الإنتاج."},
