...
9. [Périmètre par type de liste (filtres & criblage restreint)](#9-périmètre-par-type-de-liste-filtres--criblage-restreint)
10. [Ergonomie du flux de travail (dashboard)](#10-ergonomie-du-flux-de-travail-dashboard)
...
**Endpoints** : `GET /api/alerts` (file de travail triée par risque, filtres statut/assigné/**type de liste** `list_type` — CSV de types `WATCHLIST_*`, valeur spéciale `UNKNOWN` pour les alertes antérieures sans type), `GET /api/alerts/{id}` (détail + decision_tree + historique), `POST /api/alerts/{id}/assign|comment|escalate|propose|validate`.
...
Le type de liste d'origine (`WATCHLIST_OFAC`, `WATCHLIST_UN`, `WATCHLIST_DGT`, `WATCHLIST_EU`, `WATCHLIST_PEP`, `WATCHLIST_OFSI`, `WATCHLIST_SSIE`) est **dénormalisé** sur les alertes, le journal d'audit et la liste blanche (colonne `list_type`, renseignée à l'écriture).
...
* **Onglet « ⚙️ Paramètres »** (admin) : les 7 réglages à chaud y sont regroupés (ils étaient enfouis dans le sous-onglet Homologation).
* **Onglet Alertes** : deux sous-onglets — File de Travail et Liste Blanche.
* **Notifications intégrées** : plus aucun popup natif du navigateur — confirmations et saisies réglementaires (commentaires de proposition/validation 4-yeux, motifs de révocation et de rejet) passent par des modales intégrées avec zone de texte, les résultats par des toasts non bloquants.
