...
S'y ajoutent des **parseurs dédiés aux formats des sources officielles** (voir la section « Synchronisation Automatique des Sources » ci-dessous) : registre DGT des gels des avoirs (JSON), liste consolidée UE FSF (XML), liste consolidée ONU (XML), dataset PEP OpenSanctions (CSV), liste UK OFSI `ConList` (CSV) et liste suisse SECO (XML SESAM ou CSV OpenSanctions) — tous utilisables aussi bien par les synchronisations que par l'upload manuel du dashboard.
