...
// Icône du sprite SVG de index.html : trait monochrome héritant de la couleur
// du texte — l'interface n'utilise plus d'emoji.
...
const LIST_TYPE_LABELS = {
 ...
 WATCHLIST_SSIE: "SSIE",
 WATCHLIST_ADB: "Banque asiatique de développement — exclusions",
 WATCHLIST_IADB: "Banque interaméricaine de développement — exclusions",
 WATCHLIST_EBRD: "BERD — entités inéligibles",
 WATCHLIST_AFDB: "Banque africaine de développement — exclusions",
 WATCHLIST_JAPAN_MOF: "Japon — gels des avoirs (MOF)",
 WATCHLIST_MAS: "Singapour — désignations TSOFA (MAS)",
 WATCHLIST_NZ: "Nouvelle-Zélande — registre sanctions Russie",
 WATCHLIST_NL_TERROR: "Pays-Bas — liste nationale terrorisme",
 WATCHLIST_BE_TERROR: "Belgique — liste nationale de gel",
 WATCHLIST_IL_NBCTF: "Israël — organisations terroristes",
 WATCHLIST_UA_NSDC: "Ukraine — sanctions NSDC",
 WATCHLIST_AE_TERROR: "Émirats arabes unis — liste nationale de terrorisme",
 WATCHLIST_SA_TERROR: "Arabie saoudite — liste nationale de terrorisme",
 WATCHLIST_QA_NCTC: "Qatar — registre national des sanctions",
 WATCHLIST_EG_TERROR: "Égypte — liste nationale de terrorisme",
 WATCHLIST_TR_MASAK: "Türkiye — gels d'avoirs MASAK",
 WATCHLIST_ID_DTTOT: "Indonésie — liste DTTOT",
 WATCHLIST_ZA_FIC: "Afrique du Sud — sanctions financières ciblées",
 WATCHLIST_TN_CNLCT: "Tunisie — liste nationale antiterroriste",
 WATCHLIST_MC_FREEZE: "Monaco — gels de fonds",
 WATCHLIST_CZ_TERROR: "Tchéquie — désignations antiterroristes",
 WATCHLIST_US_FTO: "États-Unis — organisations terroristes étrangères (FTO)",
 WATCHLIST_GB_FCDO: "Royaume-Uni — liste de sanctions FCDO",
 WATCHLIST_GB_PROSCRIBED: "Royaume-Uni — organisations terroristes proscrites",
 WATCHLIST_AU_DFAT: "Australie — liste consolidée DFAT",
 WATCHLIST_WB_DEBARRED: "Banque mondiale — fournisseurs exclus (voie ouverte)",
 WATCHLIST_IL_CRYPTO: "Israël — portefeuilles crypto désignés",
...
const SOURCE_FAMILIES = {
 sanctions: " Sanctions officielles (gel des avoirs)",
 regulatory: " Alertes de régulateurs (signal, pas de gel)",
 debarment: " Exclusions de bailleurs multilatéraux",
...
const SOURCE_CATALOG = [
 ...
 { key: "ae_terror", alias: "AETERROR", label: "EAU (terrorisme)", family: "sanctions", desc: "Désignation nationale émirienne (RCSNU 1373) (via OpenSanctions)." },
 { key: "sa_terror", alias: "SATERROR", label: "Arabie saoudite (terrorisme)", family: "sanctions", desc: "Désignation nationale saoudienne (RCSNU 1373) (via OpenSanctions)." },
 { key: "qa_nctc", alias: "QANCTC", label: "Qatar NCTC", family: "sanctions", desc: "Désignation nationale qatarienne (comité national de lutte contre le terrorisme) (via OpenSanctions)." },
 { key: "eg_terror", alias: "EGTERROR", label: "Égypte (terrorisme)", family: "sanctions", desc: "Désignation nationale égyptienne (RCSNU 1373) (via OpenSanctions)." },
 { key: "tr_masak", alias: "TRMASAK", label: "Türkiye MASAK", family: "sanctions", desc: "Gel d'avoirs prononcé par la cellule de renseignement financier turque (via OpenSanctions)." },
 { key: "id_dttot", alias: "IDDTTOT", label: "Indonésie DTTOT", family: "sanctions", desc: "Désignation nationale indonésienne (terroristes et organisations terroristes) (via OpenSanctions)." },
 { key: "za_fic", alias: "ZAFIC", label: "Afrique du Sud FIC", family: "sanctions", desc: "Sanction financière ciblée sud-africaine (Financial Intelligence Centre) (via OpenSanctions)." },
 { key: "tn_cnlct", alias: "TNCNLCT", label: "Tunisie CNLCT", family: "sanctions", desc: "Désignation nationale tunisienne (commission nationale de lutte contre le terrorisme) (via OpenSanctions)." },
 { key: "mc_freezes", alias: "MCFREEZE", label: "Monaco (gels)", family: "sanctions", desc: "Gel de fonds prononcé par la Principauté de Monaco (via OpenSanctions)." },
 { key: "cz_terror", alias: "CZTERROR", label: "Tchéquie (terrorisme)", family: "sanctions", desc: "Désignation nationale tchèque (règlement gouvernemental) (via OpenSanctions)." },
 { key: "us_fto", alias: "USFTO", label: "US FTO", family: "sanctions", desc: "Désignation d'organisation terroriste étrangère (Département d'État) (via OpenSanctions)." },
 { key: "gb_fcdo", alias: "GBFCDO", label: "UK FCDO", family: "sanctions", desc: "Désignation du Foreign, Commonwealth & Development Office (via OpenSanctions)." },
 { key: "gb_proscribed", alias: "GBPROSC", label: "UK organisations proscrites", family: "sanctions", desc: "Organisation proscrite au titre du Terrorism Act 2000 (via OpenSanctions)." },
 { key: "au_dfat", alias: "AUDFAT", label: "Australie DFAT", family: "sanctions", desc: "Sanction australienne (ONU transposée + désignations autonomes) (via OpenSanctions)." },
 { key: "worldbank_os", alias: "WBDEBAR", label: "Banque mondiale (exclusions)", family: "debarment", desc: "Exclusion des marchés financés par le Groupe de la Banque mondiale (via OpenSanctions)." },
 { key: "il_crypto", alias: "ILCRYPTO", label: "Israël (crypto)", family: "sanctions", desc: "Portefeuille crypto désigné par le ministère de la Défense israélien (via OpenSanctions)." },
];
...
function showToast(message, type = "info", durationMs = 4500) {
 ...
 const icons = { success: uiIcon("check-circle"), error: uiIcon("warning"), info: uiIcon("info") };
...
// Sélecteurs « Liste de rattachement » de l'ajout manuel (unitaire + lot) :
// vide = snapshot manuel générique historique, sinon snapshot d'ajouts
// manuels dédié à la liste choisie (l'entité compte alors dans cette liste).
function populateManualListSelects() {
 const options = ['<option value="">Générique (par défaut)</option>']
 .concat(Object.entries(LIST_TYPE_LABELS)
 .filter(([type]) => type !== "CLIENT_BASE")
 .map(([type, label]) => `<option value="${type}">${escapeHtml(label)}</option>`))
 .join("");
 document.querySelectorAll(".manual-list-select").forEach(sel => {
 if (!sel.options.length) sel.innerHTML = options;
 });
}

// Ajout par lot : une entité par ligne « Type;Nom;Alias1,Alias2;Pays »
async function handleManualBatch() {
 populateManualListSelects();
 const rawLines = (document.getElementById("manual-batch-lines")?.value || "")
 .split("\n").map(l => l.trim()).filter(Boolean);
 const resultBox = document.getElementById("manual-batch-result");
 if (!rawLines.length) {
 showToast("Saisissez au moins une ligne (Type;Nom principal;Alias;Pays).", "error");
 return;
 }
 const entities = [];
 const malformed = [];
 rawLines.forEach((line, i) => {
 const [type, name, aliases, country] = line.split(";").map(p => (p || "").trim());
 if (!type || !name || !["I", "E", "V", "O"].includes(type.toUpperCase())) {
 ...
 if (malformed.length) {
 showToast(`${malformed.length} ligne(s) mal formée(s) (attendu : Type;Nom;Alias;Pays) — rien n'a été envoyé.`, "error");
 if (resultBox) {
 resultBox.classList.remove("hidden");
 resultBox.innerHTML = malformed.map(m =>
 `<div style="color: var(--color-alert); font-size: 0.82rem;">Ligne ${m.line} : ${escapeHtml(m.text)}</div>`).join("");
 }
 return;
 }
 const btn = document.getElementById("manual-batch-btn");
 if (btn) { btn.disabled = true; btn.textContent = "Ajout en cours…"; }
 try {
 const response = await apiFetch("/api/watchlist/entities/batch", {
 method: "POST",
 headers: { "Content-Type": "application/json" },
 body: JSON.stringify({
 list_type: document.getElementById("manual-batch-list-type")?.value || null,
 entities,
 }),
 });
 const data = await response.json();
 if (!response.ok) {
 showToast("Erreur : " + (typeof data.detail === "string" ? data.detail : "lot refusé."), "error");
 return;
 }
 showToast(data.message, data.rejected.length ? "info" : "success");
 if (resultBox) {
 resultBox.classList.remove("hidden");
 resultBox.innerHTML =
 data.added.map(a => `<div style="font-size: 0.82rem;">${escapeHtml(a.primary_name)} — ${escapeHtml(a.entity_id)}</div>`).join("")
 + data.rejected.map(r => `<div style="color: var(--color-alert); font-size: 0.82rem;">${escapeHtml(r.primary_name || "?")} : ${escapeHtml((r.errors || []).join(", "))}</div>`).join("");
 }
 ...
 showToast("Erreur réseau pendant l'ajout par lot.", "error");
 } finally {
 if (btn) { btn.disabled = false; btn.textContent = "Ajouter le lot"; }
 }
}
...
async function handleManualEntity(event) {
 ...
 const payload = {
 list_type: document.getElementById("manual-list-type")?.value || null,
 entity_type: type,
...
async function fetchSyncConfig() {
 ...
 : " Synchronisation automatique désactivée — aucune source ne part toute seule (un administrateur l'active plus bas).";
...
function renderSourcesTable(cfg) {
 ...
 // Compacité : la description passe en infobulle, la ligne tient
 // sur une hauteur de texte — 25 sources se parcourent sans friction.
 rows.push(`<tr>
 <td><strong title="${escapeHtml(s.desc)}" style="cursor: help;">${escapeHtml(s.label)}</strong></td>
 <td>${enabled
 ...
 <td><div id="src-state-${s.key}">${state}</div><div class="section-desc" style="font-size: 0.72rem; margin-top: 0.1rem;">prochaine : ${escapeHtml(nextRun)}</div></td>
 <td><button class="btn btn-primary" style="padding: 0.25rem 0.6rem; font-size: 0.78rem;"
...
function renderCronSchedules(cfg) {
 ...
 <td id="cron-state-${source}">${sourceStateCell(source)}</td>
 </tr>`;
 }).join("");
}
...
function sourceStateCell(source) {
 // Jeton serveur = `sync:<moteur en minuscules>` (ex. sync:ofacnonsdn) ;
 // les appelants passent tantôt l'alias (OFACNONSDN), tantôt la clé de
 // config (ofac_nonsdn) : on compare en alphanumérique minuscule.
 const norm = s => String(s || "").toLowerCase().replace(/[^a-z0-9]/g, "");
 const op = (_activeOps || []).find(o => String(o.token || "").startsWith("sync:")
 && norm(String(o.token).slice(5)) === norm(source));
...
// Rafraîchit les colonnes « État » des deux tableaux (sources + planification)
// sans toucher au reste du DOM : les saisies de l'utilisateur survivent au tick.
function updateSyncStateCells() {
 for (const source of Object.keys(CRON_SOURCE_KEYS)) {
 const cronCell = document.getElementById(`cron-state-${source}`);
 if (cronCell) cronCell.innerHTML = sourceStateCell(source);
 const srcCell = document.getElementById(`src-state-${source}`);
 if (srcCell) srcCell.innerHTML = sourceStateCell(source);
 }
}

// Lance en un clic une synchronisation de TOUTES les sources activées.
// Chaque source part comme un job indépendant (dedupe serveur : une source
// déjà en cours répond 409 et est simplement comptée comme telle).
async function syncAllEnabledSources() {
 const cfg = _lastSourceState;
 if (!cfg) { showToast("Configuration des sources non chargée.", "error"); return; }
 const enabled = SOURCE_CATALOG.filter(s => (cfg[s.key] || {}).enabled);
 if (!enabled.length) {
 showToast("Aucune source activée : activez-en dans la carte Synchronisation Automatique.", "info");
 return;
 }
 const btn = document.getElementById("sync-all-btn");
 if (btn) { btn.disabled = true; btn.textContent = "Lancement…"; }
 let launched = 0, busy = 0, failed = 0;
 for (const s of enabled) {
 try {
 const response = await apiFetch("/api/sync/run", {
 method: "POST",
 headers: { "Content-Type": "application/json" },
 body: JSON.stringify({ source: s.alias }),
 });
 if (response.status === 409) busy += 1; // déjà en cours : rien à faire
 else if (response.ok) launched += 1;
 else failed += 1;
 } catch (e) { failed += 1; }
 }
 if (btn) { btn.disabled = false; btn.textContent = "Synchroniser les sources activées"; }
 const parts = [`${launched} lancée(s)`];
 if (busy) parts.push(`${busy} déjà en cours`);
 if (failed) parts.push(`${failed} en échec`);
 showToast(`Synchronisation globale : ${parts.join(", ")}.`, failed ? "error" : "success");
...
async function handleScreening(event) {
 ...
 restrictionNote.innerHTML = `<small style="color: var(--color-warning);"> Criblage restreint aux listes : <strong>${escapeHtml(data.screening_lists.map(listTypeLabel).join(", "))}</strong> (tracé dans le journal d'audit)</small>`;
...
function viewAuditLogDetail(logId) {
 ...
 ? `<div style="color: var(--color-warning);"> Criblage restreint : <strong>${escapeHtml(tree.screening_lists_restriction.map(listTypeLabel).join(", "))}</strong></div>`
 : ""}
 </div>
 </div>
...
async function purgeFailedSnapshots() {
 ...
 if (btn) {
 btn.disabled = false;
 btn.innerHTML = " Purger les imports erronés";
 }
 }
}
...
function renderWatchlistDetails(item) {
 ...
 <button class="btn-secondary" onclick="showWatchlistEntityEditForm()"> Modifier la fiche</button>
 </div>`
 : "";
...
async function loadEntityRelations(entityId) {
 ...
 <strong>Règle des 50 %</strong> — détention majoritaire par
...
function showWatchlistEntityEditForm() {
 ...
 ? `<p class="section-desc" style="color: var(--color-warning, #b8860b);"> Fiche issue d'une source synchronisée : la prochaine synchronisation de la liste remplacera ce snapshot et écrasera ces modifications. Le journal des modifications, lui, est conservé.</p>`
 : "";
...
function renderUsersTable(users) {
 ...
 <button class="btn btn-sm" onclick="openEditUserModal(${u.id})" style="background: var(--surface-3); margin-right: 6px;"> Éditer</button>
 ${!isSelf ? `<button class="btn btn-sm" onclick="deleteUserAccount(${u.id}, '${escapeHtml(u.username)}')" style="background: rgba(239, 68, 68, 0.2); color: var(--danger-soft-text); border: 1px solid rgba(239, 68, 68, 0.3);"> Supprimer</button>` : ''}
 </td>
 </tr>
 `;
 }).join("");
...
async function generateTestPanel() {
 ...
 btn.disabled = false;
 btn.textContent = " Générer un panel";
 }
}
...
function renderBacktestReport(report) {
 ...
 <p class="section-desc" style="color: var(--color-warning);"> Aucun cahier de tests n'a été exécuté sur ce snapshot. Recommandé avant toute mise en production (étape 3).</p>`;
 return;
 }
 ...
 <button class="btn btn-sm btn-secondary" onclick="bulkGoodGuys()"> Good Guy (liste blanche) sur la sélection</button>
 </div>
 ...
 : `<p class="section-desc" style="color: var(--color-warning);"> Le dernier cahier de tests signale un écart de ${report.gap_pct} % (seuil : ${report.threshold_pct} %). Posez des Good Guys ou des exclusions puis relancez-le avant d'approuver.</p>`;
 }
}
...
async function openAlertModal(alertId) {
 ...
 actionsHtml += `<button class="btn btn-sm btn-primary" onclick="validateAlertDecision(true)">✓ Valider (4-yeux)</button>`;
 actionsHtml += `<button class="btn btn-sm" style="background: rgba(239,68,68,0.2); color: var(--danger-soft-text);" onclick="validateAlertDecision(false)">↩ Refuser & renvoyer</button>`;
 } else {
 ...
 actionsHtml += `<button class="btn btn-sm btn-secondary" onclick="openWhitelistModal('${escapeHtml(a.client_id || "")}', '${escapeHtml(a.watchlist_entity_id)}', '${escapeHtml(a.client_name)}', '${escapeHtml(a.watchlist_name)}')"> Mettre en liste blanche</button>`;
 }
 }
...
const DASHBOARD_WIDGETS = {
 ...
 "chart-lists": { cat: "charts", icon: uiIcon("chart"), title: "Fiches en production par liste",
 render: (body, d) => renderListsBarChart(body.id, (d.kpi.lists || {}).production_entities_by_type || {}) },
 "chart-status": { cat: "charts", icon: uiIcon("pie-chart"), title: "Répartition des alertes",
 render: (body, d) => {
 ...
 "table-workload": { cat: "tables", icon: uiIcon("briefcase"), title: "Charge par analyste", render: renderWorkloadWidget },
 "table-quality": { cat: "tables", icon: uiIcon("activity"), title: "Qualité des données clients", render: renderQualityWidget },
 "table-history": { cat: "tables", icon: uiIcon("clock"), title: "Derniers criblages", render: renderHistoryWidget },
 "table-batch": { cat: "tables", icon: uiIcon("package"), title: "Campagnes batch", render: renderBatchWidget },

 "tile-whitelist": { cat: "kpi", icon: uiIcon("heart"), title: "Liste blanche", sub: "paires posées",
 fetchValue: async () => {
...
function _hydrateDashboardWidgets() {
 ...
 if (!el) continue;
 if (def.fetchValue) {
 // Tuile à valeur asynchrone (compteur hors /api/kpi + /api/counters)
 def.fetchValue().then(v => { el.textContent = String(v); })
 .catch(() => { el.textContent = "—"; });
 } else {
 el.textContent = String(def.value(d));
 }
 } else {
...
async function renderQualityWidget(body) {
 ...
 <div class="dash-widget-note">${below ? " Sous le seuil d'alerte" : "Score global de complétude"} · ${(data.snapshot.record_count || 0).toLocaleString(uiLocale())} fiches</div>
...
async function renderHistoryWidget(body) {
 ...
 const items = (await response.json()).items || [];
 body.innerHTML = items.length
 ? '<ul class="home-list">' + items.map(h => `
 <li onclick="switchTab('audit')">
 <span class="item-main">${escapeHtml(h.client_name || h.client_id || "?")} — ${escapeHtml(statusLabel(h.status))}</span>
 <span class="item-meta">${h.final_score != null ? h.final_score + " · " : ""}${formatDateTime(h.timestamp)}</span>
 </li>`).join("") + '</ul>'
 : '<div class="dash-widget-empty">Aucun criblage exécuté.</div>';
 } catch (e) {
 body.innerHTML = '<div class="dash-widget-empty">Historique indisponible.</div>';
 }
}

async function renderBatchWidget(body) {
 try {
 const response = await apiFetch("/api/batch/campaigns", { silent: true });
 if (!response.ok) { body.innerHTML = '<div class="dash-widget-empty">Campagnes indisponibles.</div>'; return; }
 const items = ((await response.json()).items || []).slice(0, 5);
 body.innerHTML = items.length
 ...
 body.innerHTML = '<div class="dash-widget-empty">Campagnes indisponibles.</div>';
 }
}
...
function fpRuleEditorHtml(rule) {
 ...
 ${!isNew ? `<button class="btn" style="background: rgba(239,68,68,0.2); color:var(--danger-soft-text);" onclick="deleteFpRule()"> Supprimer</button>` : ""}`;
 ...
 ${!isSubmitter ? `<button class="btn btn-primary" onclick="validateFpRule()">✓ Valider & mettre en production (4-yeux)</button>` : ""}
 <button class="btn" style="background: rgba(239,68,68,0.2); color:var(--danger-soft-text);" onclick="rejectFpRule()">↩ Renvoyer en brouillon</button>`;
 } else if (status === "ACTIVE") {
 actions = `
 <button class="btn btn-secondary" onclick="editFpRuleVersion()"> Modifier (nouvelle version)</button>
 <button class="btn" style="background: var(--surface-3);" onclick="toggleFpRule()">${rule.enabled ? " Désactiver" : "▶ Activer"}</button>`;
 }
 return `
 ${branchBanner}
...
function fpRuleNlSectionHtml() {
 ...
 <label style="font-weight: 600;"> Décrire la règle en langage naturel</label>
...
async function fetchActiveOperations() {
 ...
 // État vivant par source : on ne patch QUE les cellules d'état.
 // Reconstruire le tableau ici écrasait les cases cochées et les
...
async function openClient360(clientId) {
 ...
 <li style="font-size: 0.83rem;"> ${escapeHtml(p.watchlist_name || p.watchlist_entity_id)} — ${escapeHtml(p.state)}${p.expires_at ? " · expire " + formatDate(p.expires_at) : ""}</li>`).join("");
...
async function fetchActivityReport() {
 ...
 tableEmpty(tbody, 3, "Erreur réseau.", "");
 }
}
...
async function fetchWorkload() {
 ...
 tableEmpty(tbody, 9, "Erreur réseau.", "");
 }
}
